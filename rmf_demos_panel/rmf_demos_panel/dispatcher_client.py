    ...
    def __convert_task_state_msg(self, json_obj):
        ...
        if "active" in json_obj:
            ...
            task_state["description"] = json_obj["phases"][str(
                active_phase)]["detail"]
            events = json_obj["phases"][str(active_phase)]["events"].values()
            # convert  dict_values
            events = list(events)
            if len(events) > 1:
                events = events[1:]
            ...
            for event in events:
                if event["status"] == "completed":
                    completed_events += 1
            task_state["actions"] = events
            task_state["progress"] = f"{completed_events}/{num_events}"
        ...
        try:
            ...
            if task_json["task_type"] == "Clean":
                ...
                request["description"]["dropoff"] = {
                    ...
                    "payload": []}
            elif task_json["task_type"] == "go_to_place":
                go_to_description = {'waypoint': desc["waypoint"]}
                go_to_activity = {
            ...
            elif task_json["task_type"] == "zc_action":
                description = {}
                description['category'] = 'zc_action'
                description['phases'] = []
                activities = []
            ...
            elif task_json["task_type"] == "zc_delivery":
                description = {}
                description['category'] = 'zc_delivery'
                description['phases'] = []
                activities = []
                for action in desc['actions']:
                    match action['category']:
                        case "go_to_place":
                            place = {"waypoint": action['description']['waypoint']}
                            try:
                                
                                place["orientation"] = float(action['description']['orientation']) * 3.14159 / 180.0
                            except KeyError:
                                pass
                            activities.append({
                                'category': 'go_to_place',
