...
def broadcast_states():
    ...
    while rclpy.ok():
        with app.test_request_context():
            ...
            socketio.emit('task_status', tasks, namespace=ns)
            socketio.emit('robot_states', robots,  namespace=ns)
            socketio.emit('ros_time', ros_time,  namespace=ns)
...
def rmf_state_listener(port_num: str, done_fut: asyncio.Future):
    ...
    observer = AsyncRmfMsgObserver(
        ...
        node=dispatcher_client
    )
    asyncio.set_event_loop(asyncio.new_event_loop())
    loop = asyncio.get_event_loop()
    ws = observer.generate_websocket()
